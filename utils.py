...
from settings import DATA_FILE, PROGRESS_FILE, OWM_API_URL, OWM_API_KEY
...
def fetch_weather_data(city_id: int, owm_api_url: str, owm_api_key: str):
    """Helper method to fetch weather data from OWM API"""

    response = httpx.get(owm_api_url, params={
        'id': city_id,
        'appid': owm_api_key,
        'units': 'metric'
    })
...
def collect_weather_data_task(user_id: str):
    """Celery task method from async workload"""
    data_store = load_data(DATA_FILE)
    progress = load_data(PROGRESS_FILE)
    data_store[user_id] = []
    progress[user_id] = 0
    for city_id in city_ids:
        result = fetch_weather_data(city_id=city_id, owm_api_url=OWM_API_URL, owm_api_key=OWM_API_KEY)
        sleep(1.1)  # To respect the rate limit
        weather_data = WeatherData(
            user_id=user_id,
            request_datetime=str(datetime.datetime.utcnow()),  # NOQA
            city_id=result["id"],
            celsius_temperature=result["main"]["temp"],
            humidity=result["main"]["humidity"]
        ).model_dump()
        data_store[user_id].append(weather_data)
        progress[user_id] += 1
